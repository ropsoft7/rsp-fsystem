...
function _omb_theme_PROMPT_COMMAND() {
  ...
  *)
  ...
  [[ $BC == no && $BC == -1 ]] && BC=
  BC=${BC:+${_omb_prompt_teal}${_omb_prompt_bold_green} ⥋ [$BC%]}

  local python_venv
  _omb_prompt_get_python_venv

  PS1=$TITLEBAR"\n${_omb_prompt_teal}┌─${_omb_prompt_light_gray}[\u@\h]${_omb_prompt_light_gray}${_omb_prompt_light_gray} \w $(scm_prompt_info)$python_venv\n${_omb_prompt_teal}└─${_omb_prompt_bold_green}[\A]$BC $SC${_omb_prompt_teal}${_omb_prompt_bold_teal}${_omb_prompt_green}${_omb_prompt_bold_green}⥄${_omb_prompt_bold_teal}${_omb_prompt_green} "
}
